...
To additionally stop YAML from being written, you can use `config/app.php` and set `craft\services\ProjectConfig::$writeToYaml` to `false`. Project config will still check `config/project/` for changes, but it will not write changes to that directory.
