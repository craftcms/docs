...
Modules and plugins can add new sections to the control panel using the [EVENT_REGISTER_CP_NAV_ITEMS](api3:craft\web\twig\variables\Cp::EVENT_REGISTER_CP_NAV_ITEMS) event:
...
Each item within the [navItems](api3:craft\events\RegisterCpNavItemsEvent::$navItems) array can have the following keys:

- `url` – The URL that the nav item should link to. (It will be run through <api3:craft\helpers\UrlHelper::cpUrl()>.)
...
Plugins that only need to add one section can set the `$hasCpSection` property on their primary plugin class, rather than using the [EVENT_REGISTER_CP_NAV_ITEMS](api3:craft\web\twig\variables\Cp::EVENT_REGISTER_CP_NAV_ITEMS) event:
...
You can modify aspects of the plugin’s control panel nav item by overriding its [getCpNavItem()](api3:craft\base\PluginInterface::getCpNavItem()) method:
