...
updatedVersion: 5.x/reference/twig/tags.md
---
...
## `expires` <Since ver="4.10.0" feature="The “expires” Twig tag" />
...
When neither an `in` or `on` clause are used in the tag, Craft instead calls <craft4:craft\web\Response::setNoCacheHeaders()>. Using the `expires` tag multiple times in a single request will only honor the _last_ call.
