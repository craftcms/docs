...
From your plugin’s `init()` method, use <api3:craft\services\ProjectConfig::onAdd()>, [onUpdate()](api3:craft\services\ProjectConfig::onUpdate()), and [onRemove()](api3:craft\services\ProjectConfig::onRemove()) to register event listeners that are triggered when product types are added, updated, and removed from the project config.
...
::: tip
`{uid}` is a special config path token that will match any valid UID (such as ones generated by [StringHelper::UUID()](api3:craft\helpers\StringHelper::UUID())). When an event handler is called, if the path contained a `{uid}` token, the matching UID will be available via [ConfigEvent::$tokenMatches](api3:craft\events\ConfigEvent::$tokenMatches).
:::
...
::: tip
If your component config references another component config, you can ensure that the other config changes are processed first by calling [ProjectConfig::processConfigChanges()](api3:craft\services\ProjectConfig::processConfigChanges()) within your handler method.
...
Items in the project config can be added or updated using <api3:craft\services\ProjectConfig::set()>, and removed using [remove()](api3:craft\services\ProjectConfig::remove()).

::: tip
`ProjectConfig::set()` will sort all associative arrays by their keys, recursively. If you are storing an associative array where the order of the items is important (e.g. editable table data), then you must run the array through <api3:craft\helpers\ProjectConfig::packAssociativeArray()> before passing it to `ProjectConfig::set()`.
:::
...
To avoid this, always check your plugin’s schema version _in `project.yaml`_ before making project config changes. (You do that by passing `true` as the second argument when calling [ProjectConfig::get()](api3:craft\services\ProjectConfig::get()).)
...
If your plugin is storing data in both the project config and elsewhere in the database, you should listen to <api3:craft\services\ProjectConfig::EVENT_REBUILD> (added in Craft 3.1.20) to aid Craft in rebuilding the project config based on database-stored data, when the `./craft project-config/rebuild` command is run.
