...
From your plugin’s `init()` method, use <craft4:craft\services\ProjectConfig::onAdd()>, [onUpdate()](craft4:craft\services\ProjectConfig::onUpdate()), and [onRemove()](craft4:craft\services\ProjectConfig::onRemove()) to register event listeners that are triggered when product types are added, updated, and removed from the project config.
...
Items in the project config can be added or updated using <craft4:craft\services\ProjectConfig::set()>, and removed using [remove()](craft4:craft\services\ProjectConfig::remove()).
...
To avoid this, always check your plugin’s schema version _in the YAML file_ before making project config changes. (You do that by passing `true` as the second argument when calling [ProjectConfig::get()](craft4:craft\services\ProjectConfig::get()).)
...
If your plugin is storing data in both the project config and elsewhere in the database, you should listen to <craft4:craft\services\ProjectConfig::EVENT_REBUILD> to aid Craft in rebuilding the project config based on database-stored data, when the `php craft project-config/rebuild` command is run.
