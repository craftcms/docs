...
エレメントクエリを作成すると、結果を絞り込むための[パラメータ](#parameters)をセットできます。さらに、 `.all()` を呼び出して[実行](README.md#executing-element-queries)できます。[Entry](craft3:craft\elements\Entry) オブジェクトの配列が返されます。
...
| -------------------------------------------- | ----------- |
| `1`                                          | ID が 1。     |
| a [Entry](craft3:craft\elements\Entry) object | ID が 1ではない。 |
...
| a [Entry](craft3:craft\elements\Entry) object           | 2018-05-01 より前に投稿されたもの。 |
...
| a [Entry](craft3:craft\elements\Entry) object | ハンドルが `foo` のセクション内ではない。 |
...
| -------------------------------------------- | ----------------------------- |
| `1`                                          | 公開しているもの。                     |
| a [Entry](craft3:craft\elements\Entry) object | 保留しているもの（未来の投稿日がセットされた有効なもの）。 |
...
| `'*Foo'`                                       | タイトルが `Foo` で終わる。  |
| `'*Foo*'`                                      | タイトルが `Foo` を含む。   |
| a [Section](craft3:craft\models\Section) object | タイトルが `Foo` を含まない。 |
...
| -------------------------------------------- | ---------------- |
| `1`                                          | ID が 1 のタイプ。     |
| a [Entry](craft3:craft\elements\Entry) object | ID が 1 のタイプではない。 |
