...
When saving values on a custom field, you may use the [`setFieldValue()`](craft3:craft\base\ElementInterface::setFieldValue()) and [`setFieldValues()`](craft3:craft\base\ElementInterface::setFieldValues()) methods or directly access the field handle as a property on the element object. In this example, we’re setting and saving custom field values for an [Entry](craft3:craft\elements\Entry) element:
