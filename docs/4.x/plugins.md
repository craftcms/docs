...
By default, your plugin license key will be stored in the database and [project config](project-config.md). If you wish, you may move that license key to a [PHP constant or environment variable](config/README.md#control-panel-settings) and set the key using the `$MY_PLUGIN_KEY` syntax.
