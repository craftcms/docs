...
タグクエリを作成すると、結果を絞り込むための[パラメータ](#parameters)をセットできます。さらに、`.all()` を呼び出して[実行](README.md#executing-element-queries)できます。[Tag](craft3:craft\elements\Tag) オブジェクトの配列が返されます。
...
| ---------------------------------------- | ----------- |
| `1`                                      | ID が 1。     |
| a [Tag](craft3:craft\elements\Tag) object | ID が 1ではない。 |
...
| a [Tag](craft3:craft\elements\Tag) object | for the tag represented by the object. |
