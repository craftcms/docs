...
Say you have a Matrix field with a handle `documentationField`. The field has two block types: `screenshot` and `paragraph`. The following is a list of all the input types generated for this Matrix field.
...
You can upload a file in one of two ways: pass a URL to be downloaded to the server, or pass Base64-encoded file data. Using either approach, you’ll need to use the `FileInput` GraphQL input type which has the following fields:
